...
SEO Content Factory — локальный инструмент для автоматической подготовки SEO-черновиков из HTML-страниц и CSV-файлов.

Проект показывает, как повторяемую контентную задачу можно превратить в понятный процесс: загрузить входные данные, проверить их, сформировать markdown-файл, метаданные, отчёт качества и итоговую сводку.

## Что показывает проект

- Обработку одной HTML-страницы и пакетную обработку через CSV.

- Повторяемый результат: одинаковые входные данные дают одинаковые выходные файлы.

- Запуск через командную строку, API и веб-интерфейс.

- Проверку качества результата через отдельный отчёт.

- Локальное сохранение всех файлов в `outputs/`.

- Ограничения по путям и входным данным, чтобы инструмент не работал с произвольными файлами вне разрешённой области.

## Почему это важно для бизнеса

Во многих командах есть однотипные задачи: подготовить черновики, собрать метаданные, проверить структуру, выгрузить отчёт. Этот проект показывает, как такую рутину можно оформить в устойчивый локальный инструмент с понятным запуском и проверяемыми результатами.
