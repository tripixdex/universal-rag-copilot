...
class AskRequest(BaseModel):
    ...
    top_k: int = Field(default=4, ge=1, le=MAX_TOP_K)
    min_score_threshold: float = Field(default=0.07, ge=0, le=1)
    min_evidence_results: int = Field(default=1, ge=1, le=MAX_MIN_EVIDENCE_RESULTS)
...
@app.post("/run-eval")
def run_eval(_: RunEvalRequest) -> dict[str, object]:
